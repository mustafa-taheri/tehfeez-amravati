...
export const recordFeePayment = async (
  ...
  try {
    const { studentId, academicMonthId, amount, paymentMethod, remarks } =
      req.body;
...
export const getFeeCollections = async (
  ...
  try {
    ...
    const collections = await prisma.studentFeeCollection.findMany({
      ...
      include: {
        student: {
          select: { firstName: true, lastName: true, itsNumber: true },
        },
      },
    });
...
export const generateMonthlySettlement = async (
  ...
  try {
    ...
    const totalCollected = feeCollections.reduce(
      (sum, coll) => sum + Number(coll.totalReceivedAmount),
      0,
    );
...
export const lockMonthlySettlement = async (
  ...
  try {
    ...
    if (!id) {
      res
        .status(400)
        .json({ success: false, message: "Settlement ID is required." });
      return;
    }
