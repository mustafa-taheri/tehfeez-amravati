...
export const getStudentById = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    ...
    if (!student || !student.isActive) {
      res.status(404).json({ success: false, message: 'Student not found', code: 'RESOURCE_NOT_FOUND' });
      return;
    }
...
export const createStudent = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    ...
    // Minimal validation
    if (!data.itsNumber || !data.firstName || !data.lastName || !data.currentMarhalaId) {
      res.status(400).json({
        success: false,
        message: 'Validation failed.',
        errors: [{ field: 'required', message: 'itsNumber, firstName, lastName, currentMarhalaId are required' }]
      });
      return;
    }

    // Check if exists
    const existing = await prisma.student.findUnique({ where: { itsNumber: data.itsNumber } });
    if (existing) {
      res.status(409).json({ success: false, message: 'Student with ITS Number already exists', code: 'DUPLICATE_RECORD' });
      return;
    }
    ...
    const newStudent = await prisma.student.create({
      data: {
        ...
        currentMarhalaId: data.currentMarhalaId,
